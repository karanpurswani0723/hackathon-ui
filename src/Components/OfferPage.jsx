...
class OfferPage extends React.Component {
  ...
  componentDidMount() {
    fetch("https://offer-core.azurewebsites.net/offers/1570655753136")
      .then(res => res.json())
      .then(
        result => {
          this.setState({
  ...
  handleClaim(event, offerId) {
    fetch("https://offer-core.azurewebsites.net/offers/1570655753136", {
      ...
      .then(responseText => {
        if (responseText.status === 400) alert("Successfully Claimed Offer");
                ...
                <Col span={8} style={{ marginTop: 16 }}>
                  <Card title={item.offerName} bordered={true}>
                    <Layout>
                      ...
                      <Button
                        onClick={event => {
                          this.handleClaim(event, item.offerId);
                        }}
                        type="primary"
                        htmlType="submit"
                      >
